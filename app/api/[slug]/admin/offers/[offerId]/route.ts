...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ slug: string; offerId: string }> }) {
  ...
  if (body.required_points !== undefined) update.required_points = body.required_points
  ...
  const { error } = await db.from('offers').update(update).eq('id', offerId).eq('shop_id', shop.id)
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ slug: string; offerId: string }> }) {
  ...
  await db.from('offers').delete().eq('id', offerId).eq('shop_id', shop.id)
  return NextResponse.json({ success: true })
}
